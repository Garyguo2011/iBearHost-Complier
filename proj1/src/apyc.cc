...
void
fatal (const char* loc, const gcstring& msg)
{
    error (loc, "(Fatal) %s", msg.c_str ());
    exit (1);
}
...
static void
compile (const gcstring& input, const gcstring& output)
{
...
main (int argc, char* argv[])
{
    ...
    if (outfile == NULL) {
        while (i < argc) {
            ...
            compile (infile, gcstring(infile, 0, infile.size ()-3) + ".ast");
            i += 1;
        }
    } else if (i + 1 < argc)
        Usage ();
